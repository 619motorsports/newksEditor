...
- Car workspace: packed `data.acd` is safely decoded for read-only browsing, and its
  ...
  original bytes. Other packed-data authoring, skin texture creation and preview-image
  authoring, animation authoring, cross-folder shared-driver access without
