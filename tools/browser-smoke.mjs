...
const gridPreview = process.argv.includes("--grid");
const viewAxis = process.argv.includes("--view-axis");
const selectionAxis = process.argv.includes("--selection-axis");
...
if ((!modelPath && !layoutName) || !meshName || (layoutName && !assetsPath)) throw new Error("Usage: node tools/browser-smoke.mjs (--model FILE.kn5 | --workspace MANIFEST.ini --assets FOLDER) --mesh NAME [--assembled --show-hidden] [--dynamic-advance 2] [--require-shared-geometry] [--require-dynamic-shadow-refresh] [--csp-assets assettocorsa/extension/textures] [--reflection-environment SHOWROOM.kn5 --reflection-root NODE] [--seasons --year-progress 0.5 --compare-year-progress 0] [--vao FILE.vao-patch] [--lighting --weather PRESET --sun-heading 40 --sun-height 55 --compare-sun-height 10 --manual-exposure 0.35] [--shadows] [--reflection-compare] [--surface-overlay] [--grass-fx] [--rain-fx --rain-wetness 1] [--tyre-blur 0 --tyre-dirt 0 --compare-tyre-blur 1 --compare-tyre-dirt 1] [--brake-temperature 160 --brake-front-glow 64 --brake-rear-glow 24 --brake-blur 0 --compare-brake-temperature 10 --compare-brake-blur 1] [--grid] [--view-axis] [--selection-axis] [--blurred-rims] [--cockpit-resolution] [--damage-preview] [--track-camera LABEL --track-camera-position 0.5 --play-track-camera] [--driver FILE.kn5 --driver-cockpit] [--skin NAME] [--animation NAME --animation-position 0.5] [--lod auto|INDEX] [--config FILE.ini] [--input NAME=VALUE] [--port 9222] [--app-port 4173]");
...
if(blurredRims){await waitFor(`window.__apexRenderer?.rimBlurStatus.available&&window.__apexRenderer?.rimBlurStatus.blurredNodes>0`,120000);const enabled=await evaluate(`(()=>{const button=document.querySelector('#blurred-rims');if(!button||button.hidden||button.disabled)return false;if(!button.classList.contains('active'))button.click();return true;})()`);if(!enabled)throw new Error("Native blurred-rim preview was not available");await waitFor(`window.__apexRenderer?.rimBlurStatus.mode==='blurred'`);trace("native blurred rims selected");}
if(cockpitResolution){await waitFor(`window.__apexRenderer?.cockpitPreviewStatus.available`,120000);const enabled=await evaluate(`(()=>{const button=document.querySelector('#cockpit-resolution');if(!button||button.hidden||button.disabled)return false;if(button.classList.contains('active'))button.click();return true;})()`);if(!enabled)throw new Error("Native cockpit-resolution preview was not available");await waitFor(`window.__apexRenderer?.cockpitPreviewStatus.selection==='low'`);trace("native low-resolution cockpit selected");}
if(damagePreview){await waitFor(`window.__apexRenderer?.damagePreviewStatus.available&&window.__apexRenderer?.damagePreviewStatus.exactMaterials>0`,120000);const enabled=await evaluate(`(()=>{const button=document.querySelector('#damage-preview');if(!button||button.hidden||button.disabled)return false;if(!button.classList.contains('active'))button.click();return true;})()`);if(!enabled)throw new Error("Native damage preview was not available");await waitFor(`window.__apexRenderer?.damagePreviewStatus.selection==='broken'`);trace("native broken damage state selected");}
if(gridPreview){const enabled=await evaluate(`(()=>{const button=document.querySelector('#grid-preview');if(!button||button.hidden||button.disabled)return false;if(!button.classList.contains('active'))button.click();return true;})()`);if(!enabled)throw new Error("Native grid preview was not available");await waitFor(`window.__apexRenderer?.gridVisible===true`);trace("native authoring grid selected");}
if(viewAxis){const enabled=await evaluate(`(()=>{const button=document.querySelector('#view-axis');if(!button||button.hidden||button.disabled)return false;if(!button.classList.contains('active'))button.click();return true;})()`);if(!enabled)throw new Error("Native view axis was not available");await waitFor(`window.__apexRenderer?.viewAxisStatus.visible===true&&window.__apexRenderer?.viewAxisStatus.mode===2&&window.__apexRenderer?.viewAxisStatus.vertices===6`);trace("native world-origin view axis selected");}
...
if(selectionAxis){if(!selected.found)throw new Error("The selected-node axis requires a matching mesh");await waitFor(`window.__apexRenderer?.selectionAxisStatus.visible===true&&window.__apexRenderer?.selectionAxisStatus.valid===true&&window.__apexRenderer?.selectionAxisStatus.vertices===6`);trace("native selected-node axis visible");}
const selectionAxisBeforeClear=selectionAxis?await evaluate(`window.__apexRenderer.selectionAxisStatus`):null;
...
const states = { [surfaceOverlay ? "surface-overlay" : grassFx ? "grass-fx" : rainFx ? "rain-fx" : gridPreview ? "grid=on" : viewAxis ? "view-axis=on" : selectionAxis ? "selection=active" : damagePreview ? "damage=broken" : cockpitResolution ? "cockpit=low" : blurredRims ? "rims=blurred" : brakePreview ? `brake=${Number(brakeTemperature || 10)},${Number(brakeBlur || 0)}` : tyrePreview ? `tyre=${Number(tyreBlur || 0)},${Number(tyreDirt || 0)}` : seasons ? `year-progress=${yearProgress}` : vaoPath ? "vao" : reflectionCompare ? "reflections" : lighting ? "lighting" : shadows ? "shadows" : animationName ? "animation=0" : dynamicAdvance!=="" ? `dynamic=${dynamicAdvance}` : "off"]: await screenshotState(screenshotPath) };
...
if(blurredRims){await evaluate(`window.dispatchEvent(new KeyboardEvent('keydown',{key:'F1',bubbles:true,cancelable:true}))`);await waitFor(`window.__apexRenderer?.rimBlurStatus.mode==='regular'`);states["rims=regular"]=await screenshotState();await evaluate(`document.querySelector('#blurred-rims').click()`);await waitFor(`window.__apexRenderer?.rimBlurStatus.mode==='blurred'`);}
if(cockpitResolution){await evaluate(`window.dispatchEvent(new KeyboardEvent('keydown',{key:'F3',bubbles:true,cancelable:true}))`);await waitFor(`window.__apexRenderer?.cockpitPreviewStatus.selection==='high'`);states["cockpit=high"]=await screenshotState();await evaluate(`document.querySelector('#cockpit-resolution').click()`);await waitFor(`window.__apexRenderer?.cockpitPreviewStatus.selection==='low'`);}
if(damagePreview){await evaluate(`window.dispatchEvent(new KeyboardEvent('keydown',{key:'F4',bubbles:true,cancelable:true}))`);await waitFor(`window.__apexRenderer?.damagePreviewStatus.selection==='intact'`);states["damage=intact"]=await screenshotState();await evaluate(`document.querySelector('#damage-preview').click()`);await waitFor(`window.__apexRenderer?.damagePreviewStatus.selection==='broken'`);}
if(gridPreview){await evaluate(`document.querySelector('#grid-preview').click()`);await waitFor(`window.__apexRenderer?.gridVisible===false`);states["grid=off"]=await screenshotState();await evaluate(`document.querySelector('#grid-preview').click()`);await waitFor(`window.__apexRenderer?.gridVisible===true`);}
if(viewAxis){await evaluate(`document.querySelector('#view-axis').click()`);await waitFor(`window.__apexRenderer?.viewAxisStatus.visible===false&&window.__apexRenderer?.viewAxisStatus.mode===0`);states["view-axis=off"]=await screenshotState();await evaluate(`document.querySelector('#view-axis').click()`);await waitFor(`window.__apexRenderer?.viewAxisStatus.visible===true`);}
