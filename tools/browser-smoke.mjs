...
const states = { [surfaceOverlay ? "surface-overlay" : grassFx ? "grass-fx" : rainFx ? "rain-fx" : brakePreview ? `brake=${Number(brakeTemperature || 10)},${Number(brakeBlur || 0)}` : tyrePreview ? `tyre=${Number(tyreBlur || 0)},${Number(tyreDirt || 0)}` : seasons ? `year-progress=${yearProgress}` : vaoPath ? "vao" : reflectionCompare ? "reflections" : lighting ? "lighting" : shadows ? "shadows" : animationName ? "animation=0" : dynamicAdvance!=="" ? `dynamic=${dynamicAdvance}` : "off"]: await screenshotState(screenshotPath) };
