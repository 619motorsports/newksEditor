...
if(gridPreview){await evaluate(`window.__apexRenderer.setGridVisible(true)`);await waitFor(`window.__apexRenderer?.sceneStatus.gridVisible===true`);trace("native authoring grid enabled");}
if(viewAxis){await evaluate(`window.__apexRenderer.setViewAxisMode(2)`);await waitFor(`window.__apexRenderer?.viewAxisStatus.visible===true&&window.__apexRenderer?.viewAxisStatus.mode===2&&window.__apexRenderer?.viewAxisStatus.vertices===6`);trace("native world-origin view axis enabled");}
...
const states = {};
if (wireframe) {
  await waitFor(`window.__apexRenderer?.wireframe===false`);
  states["wireframe=off"] = await screenshotState(screenshotPath);
  const enabled = await evaluate(`(()=>{const button=document.querySelector('#wireframe');if(!button||button.disabled)return false;if(!button.classList.contains('active'))button.click();return true;})()`);
  if (!enabled) throw new Error("Wireframe preview was not available");
  await waitFor(`window.__apexRenderer?.wireframe===true`);
  states["wireframe=on"] = await screenshotState();
  if (states["wireframe=off"].hash === states["wireframe=on"].hash) throw new Error("Wireframe capture did not differ from the solid capture");
  await evaluate(`document.querySelector('#wireframe').click()`);
  await waitFor(`window.__apexRenderer?.wireframe===false`);
...
if(gridPreview){await evaluate(`window.__apexRenderer.setGridVisible(false)`);await waitFor(`window.__apexRenderer?.sceneStatus.gridVisible===false`);states["grid=off"]=await screenshotState();await evaluate(`window.__apexRenderer.setGridVisible(true)`);await waitFor(`window.__apexRenderer?.sceneStatus.gridVisible===true`);}
if(viewAxis){await evaluate(`window.__apexRenderer.setViewAxisMode(0)`);await waitFor(`window.__apexRenderer?.viewAxisStatus.visible===false&&window.__apexRenderer?.viewAxisStatus.mode===0`);states["view-axis=off"]=await screenshotState();await evaluate(`window.__apexRenderer.setViewAxisMode(2)`);await waitFor(`window.__apexRenderer?.viewAxisStatus.visible===true&&window.__apexRenderer?.viewAxisStatus.mode===2`);}
...
console.log(JSON.stringify({ bptcAvailable:await evaluate(`Boolean(document.querySelector('#view').getContext('webgl2').getExtension('EXT_texture_compression_bptc'))`), summary, sceneBeforeShowHidden, selectionAxisBeforeClear, selected, states, errors }, null, 2));
socket.close();
