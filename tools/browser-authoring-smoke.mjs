...
const modelPath = option("model"), configPath = option("config"), meshName = option("mesh"), propertyName = option("property"), propertyValue = option("value"), resourceName = option("resource"), resourceValue = option("resource-value"), meshField = option("mesh-field"), meshValue = option("mesh-value");
const port = Number(option("port", "9222")), reset = process.argv.includes("--reset"), exportKn5 = process.argv.includes("--export-kn5");
const editKind = meshField ? "mesh" : resourceName ? "resource" : "property", editName = meshField || resourceName || propertyName, editValue = meshField ? meshValue : resourceName ? resourceValue : propertyValue;
if (!modelPath || !configPath || !meshName || !editName || !editValue) throw new Error("Usage: node tools/browser-authoring-smoke.mjs --model FILE.kn5 --config FILE.ini --mesh NAME (--property NAME --value VALUE | --resource SLOT --resource-value VALUE | --mesh-field NAME --mesh-value VALUE) [--reset] [--port 9222]");
...
async function loadAndSelect(clear = false) {
  ...
  const found = await evaluate(`(()=>{const search=document.querySelector('#search');search.value=${JSON.stringify(meshName)};search.dispatchEvent(new Event('input',{bubbles:true}));const row=[...document.querySelectorAll('.node-row')].find(e=>e.querySelector('.node-icon')?.textContent==='◆'&&e.querySelector('.node-name')?.textContent===${JSON.stringify(meshName)});row?.click();if(!document.querySelector('#isolate').classList.contains('active'))document.querySelector('#isolate').click();document.querySelector('#frame').click();return Boolean(row)})()`);
  if (!found) throw new Error(`Mesh not found: ${meshName}`);
}
...
const meshBoolean = editKind === "mesh" && ["isTransparent", "castShadows"].includes(editName), attribute = editKind === "resource" ? "data-edit-resource" : editKind === "mesh" ? (meshBoolean ? "data-edit-mesh-boolean" : "data-edit-mesh-number") : "data-edit-property", datasetKey = editKind === "resource" ? "editResource" : editKind === "mesh" ? (meshBoolean ? "editMeshBoolean" : "editMeshNumber") : "editProperty";
const edited = await evaluate(`(()=>{const input=[...document.querySelectorAll('[${attribute}]')].find(e=>e.dataset.${datasetKey}.toLowerCase()===${JSON.stringify(editName.toLowerCase())});if(!input)return false;input.value=${JSON.stringify(editValue)};input.dispatchEvent(new Event('change',{bubbles:true}));return true})()`);
...
await loadAndSelect(false); await waitFor(`document.querySelector('#status').textContent.includes('authored edit')`); states.recovered = await capture();
...
const ui = await evaluate(`({status:document.querySelector('#status').textContent,undoDisabled:document.querySelector('#undo').disabled,redoDisabled:document.querySelector('#redo').disabled,exportLabel:document.querySelector('#export-csp').textContent,authoredInputs:document.querySelectorAll('[data-edit-property].authored,[data-edit-resource].authored,[data-edit-mesh-number].authored,[data-edit-mesh-boolean].authored').length})`);
...
console.log(JSON.stringify({ edit: { kind: editKind, name: editName, value: editValue }, states, authored: { key: authored.key, editCount: materialEditCount + meshEditCount, csp: authored.csp }, kn5Export, ui, errors }, null, 2));
socket.close();
