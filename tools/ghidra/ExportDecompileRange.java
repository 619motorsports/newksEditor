// Ghidra headless post-script. Prints an inclusive line range from one decompiled function.
...
public class ExportDecompileRange extends GhidraScript {
    @Override
    public void run() throws Exception {
        String[] arguments = getScriptArgs();
        if (arguments.length != 3) {
            throw new IllegalArgumentException("Usage: function-address first-line last-line");
        }
        ...
        try {
            ...
            println("APEX_DECOMPILE_RANGE_START|" + function.getEntryPoint() + "|" + first + "|" + last);
            for (int line = first; line <= Math.min(last, lines.length); line++) {
                println(line + "|" + lines[line - 1]);
            }
            println("APEX_DECOMPILE_RANGE_END|" + function.getEntryPoint());
        } finally {
            decompiler.dispose();
        }
    }
