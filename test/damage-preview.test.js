...
import { auditNativeDamagePreview, damagePreviewBranchActive, nativeDamageGlassValue, nativeDamageRootForPath, nativeDamageToggle, stockDamageAmount, stockDamageResponse } from "../src/damage-preview.js";
...
function material(name, shader = "ksPerPixelMultiMap_damage_dirt", resources = ["txDamage", "txDamageMask"]) {
  return { name, shader, properties: [{ name: "damageZones", value4: [0, 0, 0, 0] }], resources: resources.map((slot) => ({ slot })) };
}
...
test("sets glassDamage after either F4 state without changing the authored value", () => {
  const damage = node("DAMAGE_GLASS_FRONT_1", false), mesh = node("MESH");
  const audit = auditNativeDamagePreview(node("SCENE", true, [damage]), []);
  assert.equal(nativeDamageGlassValue([damage, mesh], audit, null, 0.25), 0.25);
  assert.equal(nativeDamageGlassValue([damage, mesh], audit, true, 0.25), 1);
  assert.equal(nativeDamageGlassValue([damage, mesh], audit, false, 0.25), 1);
  assert.equal(nativeDamageGlassValue([node("OTHER"), mesh], audit, true, 0.25), 0.25);
});
