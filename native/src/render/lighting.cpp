...
float cspLightDistanceFade(float distance,float fade_at,float fade_smooth) noexcept { const float center=std::max(0.0F,finiteOr(fade_at,0)),width=std::max(0.0F,finiteOr(fade_smooth,0)),value=std::max(0.0F,finiteOr(distance,0));if(width<=1e-6F)return value<center?1:0;return clamp((center+width*.5F-value)/width,0,1); }
...
ExponentialShadowParams cspExponentialShadowParams(const ExponentialShadowInput& i) noexcept { const float raw_range=std::isfinite(i.range)&&i.range!=0?i.range:10,source=std::max(.001F,raw_range),limit=i.vehicle_attached?120:30,max_range=i.shadow_range_authored&&i.shadow_range>0?finiteOr(i.shadow_range,source):std::min(source,limit),raw_exp=std::isfinite(i.exp_factor)&&i.exp_factor!=0?i.exp_factor:20,exp_factor=clamp(raw_exp,.001F,80),spot=std::max(0.0F,finiteOr(i.shadow_spot,0))*pi/180,automatic=10-7.5F*clamp((spot-pi*.6F)/(pi*.3F),0,1),boost=i.shadow_boost>0?finiteOr(i.shadow_boost,automatic):automatic;ExponentialShadowParams o;o.max_range=max_range;o.range_inverse=1/max_range;o.range_inverse_exp_factor=exp_factor/max_range;o.exp_factor=exp_factor;o.bias_mult=exp_factor*.3F/max_range;o.thickness_fix_mult=boost;o.thickness_fix_add=1-boost;o.boost=boost;o.clip_plane=std::max(.001F,std::isfinite(i.shadow_clip_plane)&&i.shadow_clip_plane>0?i.shadow_clip_plane:.5F);o.clip_sphere=std::max(0.0F,i.shadow_clip_sphere_authored&&std::isfinite(i.shadow_clip_sphere)?i.shadow_clip_sphere:.5F);o.extra_blur=i.shadow_extra_blur;return o; }
