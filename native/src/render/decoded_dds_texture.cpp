...
plan_decoded_texture_payload(std::span<const std::uint8_t> bytes,
  ...
  const bool bmp_prefix = bytes.size() >= 2u && bytes[0] == 'B' &&
                          bytes[1] == 'M';
  if (bmp_prefix) {
    try {
      formats::BmpLimits bmp_limits;
      bmp_limits.parse = limits;
      formats::BmpImage image =
          formats::decodeBmpRgba8(bytes, source, bmp_limits);
      DecodedTexturePlan plan;
      plan.description = {
      ...
      return failure(TextureUploadStatus::invalid, "allocation_failed",
                     "BMP decode planning has insufficient memory for bounded storage", 0U,
                     std::move(source));
    } catch (const std::exception &error) {
      return failure(TextureUploadStatus::invalid, "decode_failed", error.what(),
                     0U, std::move(source));
