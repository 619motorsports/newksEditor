...
// Portable Yebis curve approximation with the five-level bloom composite.
...
void main() {
    ...
    vec3 value = max(vec3(1.0 / 16384.0),
                     mix(vec3(luminance), rgb, parameters.saturation) * parameters.exposure);
    ...
    vec3 glare = (texture(bloom0, texture_coordinates).rgb +
                  texture(bloom1, texture_coordinates).rgb +
                  texture(bloom2, texture_coordinates).rgb +
                  texture(bloom3, texture_coordinates).rgb +
                  texture(bloom4, texture_coordinates).rgb) * parameters.bloom_scale;
    curve += (vec3(1.0) - curve) * clamp(glare, 0.0, 1.0);
    ...
    float dither = (float(dither_rgba8[dither_index]) / 255.0 - 0.5) * parameters.dither_scale;
    encoded += vec3(dither);
    vec3 output_rgb = parameters.srgb_destination != 0U ? decode_srgb(encoded) : encoded;
